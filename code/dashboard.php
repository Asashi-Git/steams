...
// --- Guard: must be connected and have a role >= 1
...
// --- Handle POST actions (delete, pin, edit) before rendering
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';

    // Admin deletes any review
    if ($action === 'delete_review' && $user['id_role'] === 2) {
        $controller->deleteReview((int)$_POST['id_review']);
    }

    // Admin pins a review
    if ($action === 'pin_review' && $user['id_role'] === 2) {
        $controller->pinReview((int)$_POST['id_review']);
    }

    // Admin deletes a user
    if ($action === 'delete_user' && $user['id_role'] === 2) {
        $controller->deleteUser((int)$_POST['id_user']);
    }

    // Critic deletes their own review
    if ($action === 'delete_own_review' && $user['id_role'] >= 1) {
